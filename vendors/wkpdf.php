    ...
    private static function _getCPU() {
        if (!self::$cpu) {
            if (`grep -i amd /proc/cpuinfo` != '') {
                self::$cpu = 'amd64';
            } else if (`grep -i intel /proc/cpuinfo` != '') {
                self::$cpu = 'i386';
            } else if (`/usr/bin/hostinfo | grep -i amd` != '') {
                self::$cpu = 'amd64';
            } else if (`/usr/bin/hostinfo | grep -i intel` != '') {
                self::$cpu = 'i386';
            } else {
                throw new Exception("WKPDF couldn't determine CPU /usr/bin/hostinfo or /proc/cpuinfo.");
            }
        }
        return self::$cpu;
    }
 ...
 */
    public function __construct($base_path) {
        $this->cmd = sprintf("%swkhtmltopdf-%s", $base_path, self::_getCPU());
        $potential = trim(`which wkhtmltopdf`);

        if (!file_exists($this->cmd) && !empty($potential)) $this->cmd = $potential;

        if (!file_exists($this->cmd)) {
            $exception = sprintf("WKPDF static executable %s was not found", htmlspecialchars($this->cmd, ENT_QUOTES));
            throw new Exception($exception);
        }

        do {
            $this->tmp = sprintf("%s%s.html", $base_path, mt_rand());
 ...
 */
    public function render() {
        $web    = $this->tmp;
        $cmd    = sprintf('%s --copies %s --orientation %s --page-size %s',
            $this->cmd,
            $this->copies,  // number of copies
            $this->orient,  // orientation
            $this->size     // page size
        );

        if ($this->toc)         $cmd .= " --toc";                   // table of contents
        if ($this->grayscale)   $cmd .= " --grayscale";             // grayscale
        if ($this->title)       $cmd .= " --title '{$this->title}'";// grayscale

        printf(sprintf("%s '%s' -", $cmd, $web) ."\n");
        $this->pdf = self::_pipeExec(sprintf("%s '%s' -", $cmd, $web)); // URL and optional to write to STDOUT

        if (strpos(strtolower($this->pdf['stderr']),'error')!==false) {
            throw new Exception(sprintf('WKPDF system error: <pre>%s</pre>', $this->pdf['stderr']));
        }
        if (empty($this->pdf['stdout'])) {
            throw new Exception(sprintf("WKPDF didnt return any data. <pre>%s</pre>", $this->pdf['stderr']));
        }
        if (((int)$this->pdf['return']) > 1) {
            throw new Exception(sprintf("WKPDF shell error, return code %s.", (int)$this->pdf['return']));
        }
    ...
    public function output($mode,$file) {
        switch ($mode) {
            ...
            case self::$PDF_SAVEFILE:
                ...
                break;
            default:
                throw new Exception(sprintf('WKPDF invalid mode "%s".', htmlspecialchars($mode,ENT_QUOTES)));
        }
        return false;
    }
}